...
$(function(){
	//查询出顶级模块信息
	$.get(contextPath + "module/queryTopModule.do",function(data){
		var $topmenuTable = $("#topmenucontent");
		var $tr = $("<tr>");
		$topmenuTable.append($tr);
		var menuId ,sysName;
		for(var i = 0;i<data.length;i++){
			var module = data[i];
			var $td = $("<td>");
			$tr.append($td);
			$td.attr("title",module.text);
			$td.attr("id",module.id); 
			if(i == 0){
				$td.addClass("topmenu selected systemName");
				menuId = module.id;
				sysName = module.text;
			}else{
				$td.addClass("topmenu systemName");
			}
			var html = '<a class="l-btn-text bannerMenu" href="javascript:void(0)">' + 
			' <p>' + 
			'<lable class="'+ module.iconCls +'"></lable>' + 
			'</p>' + 
			' <p><span style="white-space:nowrap;">' + module.text +'</span></p>' + 
			'</a>';

			$td.html(html);
		}

		generateMenu(menuId, sysName)

		// 绑定横向导航菜单点击事件
		$(".systemName").on("click", function (e) {
			generateMenu($(this).attr("id"), $(this).attr("title"));
			/*if("设备管理"==$(this).attr("title")){
				var tab = parent.$('#index_tabs').iTabs("getTab","设备管理首页");
				if(!tab){
					var params = {title:"设备管理首页",href:"console/jsp/deviceManagement/index.jsp"}; 
					addParentTab(params);
					}
			}*/
...
function generateMenu(menuId, systemName) {
	if (menuId < 8000) {
		...
		var url = contextPath + "module/querySubModule.do";
		$.get(
				url, {"levelId": "2","pid":menuId}, // 获取第一层目录
				function (data) {
...
function addParentTab(options) {
	...
	if(!parent.$('#index_tabs').iTabs('exists',title)){
...
function addParentTabWithTip(options) {
	var src, title,url;
	src = options.href;
	title = options.title;
	url = options.url;
	...
	if(!parent.$('#index_tabs').iTabs('exists',title)){
        ...
        $.iMessager.confirm("提示","该标签页已打开，需要将其关闭并重新打开吗？",function(r){
            if(r){
                parent.$('#index_tabs').iTabs('close',title);
                if(url){
					$.get(url,function(result) {
						addParentTabWithTip(options);
					});
                }else{
                    addParentTabWithTip(options);
				}
...
function modifyPwd() {
	var opts = {
			...
			height: 300,
			iconCls: 'fa fa-key',
			href: 'console/js/html/user/modifyPassword.jsp',
			buttons: [{
				text: '确定',
				iconCls: 'fa fa-save',
						...
						$.ajax({
							...
							success: function (data, response, status) {
								...
								if (data.statusCode == 200) {
									...
									$.messager.alert('提示','密码已修改，请重新登录!','info',function(){
										window.top.location.href="logout";
									});
								}else if(data.statusCode==300){
									$.iMessager.alert('操作失败！', '原始密码错误！', 'messager-error');
								}else {
