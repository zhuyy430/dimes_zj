...
<script>
	$(function() {
		...
		$('#showDevicesDialog').dialog({
		    ...
		    closed: true,
		    cache: false,
		    href: 'console/jsp/deviceManagement/maintenancePlan_showDevices.jsp',
		    modal: true,
		    buttons:[{
		        text:'保存',
		        handler:function(){
		        	var devices = $('#deviceTable').iDatagrid('getSelections');
	           		var codesArray = new Array();
	           			for(var i = 0;i < devices.length;i++){
	           				codesArray.push(devices[i].code);
	           			}
	           			var codesStr = JSON.stringify(codesArray);
	           			codesStr = codesStr.replace('[','');
	           			codesStr = codesStr.replace(']','');
	           			codesStr = codesStr.replace(/"/g,'');
	           		$('#deviceCodes').iTextbox('setValue',codesStr);
		    		$('#showDevicesDialog').dialog("close");
		    		if(codesStr.indexOf(",")==-1){
		    			$.get("device/queryDeviceByCode.do",{name:'code',value:codesStr},function(result){
		    				$("#deviceName").iTextbox("setValue",result.name);
		    				$("#unitType").iTextbox("setValue",result.unitType);
		    			});
		    		}else{
		    			$("#deviceName").iTextbox("setValue",'');
	    				$("#unitType").iTextbox("setValue",'');
		    		}
		        }
		    },{
		        text:'关闭',
...
<div data-toggle="topjui-layout" data-options="fit:true">
	<div
		...
		<form id="addDeviceForm" method="post">
			<div title="新增保养计划" data-options="iconCls:'fa fa-th'">
				<div class="topjui-fluid">
					...
					<div class="topjui-row">
						<div class="topjui-col-sm12">
							<label class="topjui-form-label" style="text-align: center;">保养周期</label>
							<div class="topjui-input-block"
								...
								<div class="cycleType">
									<input type="radio" name="cycleType" value="forDay" id="forDay" checked="checked" />
									<label for="forDay">每天</label>
								</div>
								<div class="cycleType">
								...
								<div class="cycleType">
									...
									<input id="forMonthValue" data-toggle="topjui-combobox"
										...
										,{text:'26'},{text:'27'},{text:'28'},{text:'29'},{text:'30'},{text:'31'}]"><label style="margin-left:5px;font-size:14px;">日</label>
								</div>
					...
					<div class="topjui-row">
						...
						<div class="topjui-col-sm6">
							<label class="topjui-form-label" style="text-align: center;">预计用时(分钟)</label>
							<div class="topjui-input-block">
								<input type="text" name="expectTime"
