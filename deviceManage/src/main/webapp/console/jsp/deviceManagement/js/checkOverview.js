...
function showReport(){
	$.get("checkOverview/separateOverviewReport.do",{
		from:$('#from').val(),
		to:$('#to').val()
},function(result){
...
//导出数据
function exportData(){
	window.location.href='checkOverview/exportOverview.do?from=' + $('#from').val() +"&to="+$('#to').val();
}
...
function showData(){
	...
	$('#pg').pivotgrid({
    	title:'',
    	height:500,
    	url:'checkOverview/overviewReport.do',
    	method:'get',
    	pivot:{
    		rows:['category'],
    		columns:['txt'],
