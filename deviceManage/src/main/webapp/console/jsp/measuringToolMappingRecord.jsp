...
$(function() {
	...
	$("#searchBtn").click(function(){
		  $("#departmentDg").iDatagrid("reload",'measuringToolMappingRecord/queryMeasuringToolMappingRecordByDeviceSiteIdandSearch.do?'+$("#searchForm").serialize());
	});
});
</script>
<body>
			...
			<div data-toggle="topjui-layout" data-options="fit:true">
				<div
					...
					<table data-toggle="topjui-datagrid"
                       ...
                       url:'measuringToolMappingRecord/queryMeasuringToolMappingRecordByDeviceSiteId.do',
                       singleSelect:true,
                       fitColumns:true,
                       pagination:true,
                       parentGrid:{
			           ...
			           childTab: [{id:'southTabs'}]">
						<thead>
                         ...
                         if (value) {
								...
								<th
										data-options="field:'equipmentCode',title:'量具序列号',width:'120px',align:'center',sortable:false,formatter:function(value,row,index){
                         if (row.equipment) {
								...
								<th
										data-options="field:'equipmentName',title:'量具名称',width:'120px',align:'center',sortable:false,formatter:function(value,row,index){
                        			 if (row.equipment) {
...
<sec:authorize access="hasAuthority('ADD_MEASURINGTOOLMAPPINGRECORD')">
       ...
       dialog:{
           ...
           width:600,
           height:500,
           href:'console/jsp/measuringToolMappingRecord_add.jsp',
           buttons:[
           	{text:'保存',handler:function(){
           			var equipmentId = $('#equipment').val();
           			if(!equipmentId){
           				$.iMessager.alert('提示','请选择量具');
           				return false;
           			}
           			$.get('measuringToolMappingRecord/addMeasuringToolMappingRecord.do',{
...
<sec:authorize access="hasAuthority('EDIT_MEASURINGTOOLMAPPINGRECORD')">
		<a href="javascript:void(0)" data-toggle="topjui-menubutton"
			data-options="method: 'openDialog',
            extend: '#departmentDg-toolbar',
            ...
            dialog: {
                ...
                width: 600,
                height: 400,
                href: 'console/jsp/measuringToolMappingRecord_edit.jsp',
                url:'measuringToolMappingRecord/queryMeasuringToolMappingRecordById.do?id={id}',
                 buttons:[
           	{text:'编辑',handler:function(){
           		var equipmentId = $('#equipmentId').val();
           			...
           			$.get('measuringToolMappingRecord/updateMeasuringToolMappingRecord.do',{
...
<sec:authorize access="hasAuthority('DEL_MEASURINGTOOLMAPPINGRECORD')">
		<a href="javascript:void(0)" data-toggle="topjui-menubutton"
			data-options="method:'doAjax',
       extend: '#departmentDg-toolbar',
       iconCls:'fa fa-trash',
       url:'measuringToolMappingRecord/deleteMeasuringToolMappingRecord.do',
       grid: {uncheckedMsg:'请先勾选要删除的数据',id:'departmentDg',param:'id:id'},
       onSuccess:function(){$('#departmentTg').iTreegrid('reload');}">删除</a>
</sec:authorize>
<sec:authorize access="hasAuthority('RELIEVE_MEASURINGTOOLMAPPINGRECORD')">
		<a href="javascript:void(0)" data-toggle="topjui-menubutton"
			data-options="method:'doAjax',
       extend: '#departmentDg-toolbar',
       iconCls:'fa fa-chain-broken',
       url:'measuringToolMappingRecord/unbindMeasuringToolMappingRecord.do',
       grid: {uncheckedMsg:'请先勾选要操作的数据',id:'departmentDg',param:'id:id'}">解除关联</a>
         ...
         <form id="searchForm" method="post" style="display:inline;float: right;">
			...
			<span> <label for="searchChange">过滤字段: </label> <input
				...
				valueField:'id',
				textField:'name',
				data:[{id:'equipment.code',name:'量具序列号'},
					  {id:'equipment.equipmentType.name',name:'量具名称'},
