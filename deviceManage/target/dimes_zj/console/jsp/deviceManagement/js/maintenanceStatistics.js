...
function showReport(){
	$.get("maintenanceStatistics/separateStatisticsReport.do",{
		from:$('#from').val(),
		to:$('#to').val(),
		cycle:$('#cycle').val()
},function(result){
...
//导出数据
function exportData(){
	window.location.href='maintenanceStatistics/exportStatistics.do?from=' + $('#from').val() +"&to="+$('#to').val()+"&cycle="+$('#cycle').val();
}
...
function showData(){
	...
	$('#pg').pivotgrid({
    	...
    	toolbar:"#toolbar",
    	height:500,
    	url:'maintenanceStatistics/statisticsReport.do',
    	method:'get',
    	toolbar:'#toolbar',
    	pivot:{
    		rows:['category'],
