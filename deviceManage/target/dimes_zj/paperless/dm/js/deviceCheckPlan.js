		...
		for(var i = 0;i<projectRecords.length;i++){
          ...
          //项目代码
			...
			if(record.note!=null){
			 ...
			 note.append("<input type='text' class='noteValue' value='" + recordNote + "' style='background-color: #1C2437;color:white;border:0;height:100%;width:100%;' />");
			tr.append(note);
...
function changeColor(obj){
	...
	case "paperless/deviceRepair.jsp?param=dm":{
		$iframe.attr("src","paperless/dm/deviceRepair.jsp?param=dm");
		break;
	}
	case "paperless/deviceMessage.jsp?param=dm":{
		$iframe.attr("src","paperless/dm/deviceMessage.jsp?param=dm");
		break;
	}
	case "paperless/deviceService.jsp?param=dm":{
		$iframe.attr("src","paperless/dm/deviceService.jsp?param=dm");
		break;
	}
	case "paperless/deviceMaintain.jsp?param=dm":{
		$iframe.attr("src","paperless/dm/deviceMaintain.jsp?param=dm");
		break;
	}
	}
}