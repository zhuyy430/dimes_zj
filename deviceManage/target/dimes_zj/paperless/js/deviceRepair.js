...
function initLostTimeTable(deviceCode) {
	...
	$("#showNGRecordTable").bootstrapTable({
		...
		columns : [ {
			...
			formatter : function(value, row, index) {
				if (row.projectType) {
					console.log(row.projectType.code);
			...
			formatter : function(value, row, index) {
				if (row.projectType) {
					console.log(row.projectType.name);
...
function addLostTime(e) {
	...
	if (e == "repairAdd") {// 新增
		...
		console.log(JSON.stringify(alldata[0].url));
...
// 获取所有故障原因
/*
 * function getAllReasonAdd(){ $.ajax({ type : "post", url : contextPath +
 * "projectType/queryProjectTypesByType.do?type=breakdownReasonType", data : {},
 * cache:false, dataType : "json", success : function(data) {
 * //alert(JSON.stringify(data)); var htmlselect = "<option></option>";
 * $.each(data,function(index, Type) { htmlselect += "<option
 * value='"+Type.id+"'>"+Type.name+"</option>"; }) console.log(htmlselect);
 * $("#pressLight").empty(); $("#pressLight").append(htmlselect);
 * $("#pressLight").selectpicker('refresh');
 * 
 * $("#updatePressLight").empty(); $("#updatePressLight").append(htmlselect);
			...
			.bootstrapTable(
					{
						...
						columns : [
								 ...
								 * (value=="SYSTEMGASSIGN") { return "系统派单";
								 * }else if(value=="ARTIFICIALGASSIGN"){ return
								 * "人工派单"; }else if(value=="ASSIST"){ return
								 * "协助"; }else{ return ""; } }
								 */
								},
								{
...
function showM(id, flag) {
	...
	if (flag == "保养") {
		...
		$("#removeMaintenanceSparepartView").hide();
		$("#removeStaffMaintenanceView").hide();
		$("#updateDeviceMaintenance").attr("display","none");
