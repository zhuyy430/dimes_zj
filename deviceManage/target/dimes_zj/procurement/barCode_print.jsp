        ...
        $.get("boxBar/printQr.do",{
            ...
            while(i<data.length){
                ...
                if(data1!=undefined){
                    ...
                    var warehousingDate = content1('来料日期',(data1.warehousingDate==null?" ":data1.warehousingDate));
                    ...
                    var batchNumber = content('炉号',(data1.stoveNumber==null?" ":data1.stoveNumber));
                    var furnaceNumber = content('材料编号',(data1.furnaceNumber==null?" ":data1.furnaceNumber));
                    var amount = content('数量',formatFloat(data1.surplusNum,3));
                    var boxNum = content('钢厂',(data1.manufacturer));
                    var positionCode = content('货位',(data1.positonCode));
                    //var source = content('箱号',(data1.source));
                    ...
                    $innerBottom.append(positionCode);
                    var source='<div class="topjui-col-sm12 topjui-col-xs12">'+
                        '<label class="topjui-form-label label" style="text-align: center;">来源:</label>' +
                        '<div class="topjui-input-block data1">' + data1.source +
                        '</div>'+
                        '</div>';
                    $innerBottom.append(source);
                    $row.append($col1);
                }
                $center.append($row);
            }
